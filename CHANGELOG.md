...
- Added two plasma-scoped categories to **`packages.x86_64`** so the Plasma extras page now
  ...
  - **`### CATEGORY: Plasma — Themes`** — `kiro-plasma-sweet` (the complete Sweet Plasma global
    theme: desktoptheme, look-and-feel, aurorae, color scheme, Kvantum, SDDM), the four further
    Kiro Plasma global themes `kiro-plasma-layan`, `kiro-plasma-nord`, `kiro-plasma-whitesur`
    and `kiro-plasma-win11`, and `surfn-plasma-flow` (the Surfn Flow theme).
...
### Technical Details
- **All** pacman.confs enforce `[nemesis_repo]` at `Required DatabaseOptional` — the shipped/installed (`archiso/airootfs/etc/pacman.conf`) **and** both build-time confs (`build-scripts/pacman.conf` → build-host `/etc/pacman.conf`; `archiso/pacman.conf` → the `-C` config `mkarchiso` reads). There is no per-repo `Never` override anywhere; nemesis verification is uniform. Build-time enforcement works because the Kiro signing key is **trusted before `mkarchiso` pulls nemesis packages**: `host-prep.sh`'s `setup_kiro_keyring` installs `kiro-keyring` on the build host (providing `/usr/share/pacman/keyrings/kiro.gpg`), and Phase 7 `prepopulate_keyring` runs `--populate kiro` into the airootfs build keyring. The keyring-bootstrap problem (the keyring is itself a signed nemesis package) is solved by **seeding the key**, not by relaxing `SigLevel`.
...
- **Build-host guard:** `host-prep.sh` now has `setup_kiro_keyring` (called from `build-the-iso.sh` main alongside `setup_chaotic`/`setup_cachyos`) — installed? skip; else `pacman -S --needed kiro-keyring`. Ensures `/usr/share/pacman/keyrings/kiro.gpg` is present so `prepopulate_keyring`'s `--populate kiro` doesn't fail on a clean build host. No-op where the keyring is already installed.
...
- Wired `nemesis_repo` through `kiro-mirrorlist` in **every** config the same way `chaotic-aur`/`cachyos` already are — `Include` where the file is guaranteed present, direct `Server` only where it must bootstrap:
  - **`archiso/airootfs/etc/pacman.conf`** (shipped/installed) → `Include`. The unused **`archiso/airootfs/etc/pacman.conf.kiro`** variant matched.
  - **`archiso/pacman.conf`** (the `-C` config `mkarchiso` reads, resolving against the build host) → `Include`.
  - **`build-scripts/pacman.conf`** (copied verbatim onto the build host's `/etc/pacman.conf`) → **kept on direct `Server`, by design** — it must bootstrap-install `kiro-keyring`/`kiro-mirrorlist` *before* the mirrorlist file exists, so it can't `Include` it. This mirrors the existing deliberate choice for `[cachyos]` in the same file, and carries the same explanatory comment.
- **New host-prep step `setup_kiro_mirrorlist()`** (`build-scripts/host-prep.sh`, called from `build-the-iso.sh` `main()` right after `setup_kiro_keyring`). It writes `/etc/pacman.d/kiro-mirrorlist` on the build host with an inline `printf`/`tee` — dependency-free and idempotent (skips if the file is already there from the installed package). This is what makes the `Include` in `archiso/pacman.conf` resolve on a clean build host, the same role `setup_chaotic`/`setup_cachyos` play for their mirrorlists. `SigLevel` is unaffected (global, inherited).
- **Sequencing caveat:** `kiro-mirrorlist`'s source is pushed to `kirodubes/KIRO-PKG-BUILD-APPS`, but the nemesis_repo build flow has **not** run yet. It must be built and served from nemesis_repo **before** the next -next ISO build, or `mkarchiso` will fail to find the package (the `Include`/host-prep work resolves the *URL*, not the package's availability in the repo).
- Files: `archiso/airootfs/etc/pacman.conf`, `archiso/airootfs/etc/pacman.conf.kiro`, `archiso/pacman.conf`, `build-scripts/pacman.conf`, `build-scripts/host-prep.sh`, `build-scripts/build-the-iso.sh`, `archiso/packages.x86_64`.
...
Relatedly, **`build-scripts/host-prep.sh`** gained an **`enable_cachyos()`** host-prep function. Kiro ships `[cachyos]` commented out by default (chaotic-aur is the backstop), but a handful of CachyOS flavors — `linux-cachyos-eevdf`, `-hardened`, `-bmq`, `-cacule`, `-deckify`, `-server`, `-rt-bore` — exist **only** in the `cachyos` repo, not on chaotic-aur. When a build host has the cachyos keyring/mirrorlist installed but the repo commented out, `enable_cachyos()` uncomments just the `[cachyos]` block (awk-scoped to the section header + its commented config lines, stopping at the next blank line or section, with a `/etc/pacman.conf.kiro-bak` backup) and re-syncs — distinct from `setup_cachyos()`, which handles the from-scratch keyring+mirrorlist install. The kiro-iso-builder GUI calls it via the existing `host-prep-run.sh` dispatcher (`pkexec bash host-prep-run.sh enable_cachyos`), surfaced only when a selected kernel actually needs it — never as a nag against the intentional default.

- **`build-scripts/host-prep.sh`** — new `enable_cachyos()`; **`build-scripts/host-prep-run.sh`** — Usage comment lists the new function.
...
Crucially this is **not "all Wayland"**: **GNOME-Wayland (mutter)** and **Plasma-Wayland (kwin)** launch the installer fine — those compositors set up XWayland so root can connect; minimal compositors like **labwc** do not. So the fix in **`calamares_polkit`** (shipped by the **`calamares-next`** package, KIRO-PKG-BUILD-CALAMARES) keys on the **observable symptom, not the compositor**: it probes whether a root process can already reach the X display (`sudo -n env DISPLAY=… XAUTHORITY=… xhost`) and only when it can't does it grant access (`xhost +SI:localuser:root`, with a `trap` revoke and `DISPLAY`/`XAUTHORITY` passed through `pkexec`). On GNOME/Plasma/X11 the probe succeeds and the launcher takes the **byte-for-byte original path** — and because the grant is additive + revoked, even a false-positive probe cannot regress a working session. This also means future Wayland editions (Hyprland, niri) are handled correctly with no further code change.

- **`calamares_polkit`** (in `calamares-next`) — runtime probe + conditional `xhost` grant; original path untouched where root already reaches the display.
- **`archiso/packages.x86_64`** — added **`xorg-xhost`** (provides the `xhost` the launcher needs; tiny, harmless on X11).

Scoped to the beta chain only: **`calamares-next`** + **`kiro-iso-next`**; production `calamares` / `kiro-iso` are untouched pending validation. The Calamares Tweak Tool has the same root-under-Wayland bug and gets the identical probe wrapper in a later pass once this is proven on a `-next` ISO.
...
**What Changed**
- In **`build-scripts/build-the-iso.sh`** (`apply_editions`), when `default_session` isn't one of the selected `editions`, the fallback now follows a priority: a selected **full desktop** (DE) wins, else the flagship **`ohmychadwm`**, else the first edition. For several DEs, the first one in `editions=` wins.

**Why**
- The live ISO autologin session is `Session=` in `/etc/sddm.conf.d/kde_settings.conf`, which `apply_editions` bakes from `default_session`. The shipped `default_session="xfce"` is **not in** the standard `editions=` list, so the old guard fell back to the *first* edition — alphabetically `awesome`, or whatever led the list (e.g. `chadwm` in a `chadwm ohmychadwm` test build). That meant the live ISO never booted the flagship **ohmychadwm**, even though it was baked in. A first attempt simply preferred ohmychadwm, but that was too blunt — it made the flagship WM beat a **deliberately selected full desktop** (e.g. `plasma ohmychadwm` booted ohmychadwm, not Plasma). The final rule treats a chosen DE as the intended primary: a full desktop outranks the WMs, ohmychadwm is the default only when no DE is selected, and the first edition is the last resort. Verified: `plasma ohmychadwm`→plasma, `gnome plasma ohmychadwm`→gnome, standard TWM set→ohmychadwm, `awesome i3 qtile`→awesome.
- This does **not** touch Calamares' `kiro_displaymanager` — its installed-system default deliberately lets full DEs like XFCE rank ahead of the WMs, which is intended.

**Technical Details**
- The DE list in `apply_editions` (`xfce cinnamon plasma gnome mate budgie lxqt deepin`) **mirrors KIB's `DESKTOPS` set** in `configure_gui.py` — keep the two in sync when adding a desktop.
...
**Why**
- Plasma joins Cinnamon, MATE, GNOME, and Budgie as a selectable full desktop. The block ships the **`plasma`** group (following the "group names wherever possible" convention) plus **`konsole`** (the group ships no terminal) and **`kde-system-meta`** (KDE system applications), so Plasma is self-sufficient as a standalone session. **Plasma runs on Wayland here** — a deliberate exception: it's an opt-in edition, and the Plasma **X11 session is deprecated upstream** (removed in Plasma 6.8). The X11 session entry lives in a separate `plasma-x11-session` package (the group's `kwin-x11` is only the X11 *compositor*, not the session), which is **intentionally not shipped**. **Unbranded** (no `kiro-plasma` package), like the `gnome`/`budgie` blocks. KIB already classifies `plasma` as a desktop (in the `DESKTOPS` set) and auto-discovers the block via `list_editions()`, so the builder surfaces it with no GUI change.
